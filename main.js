...
/// Render waveforms.
/// </summary>
/// <param name="populationIndex">an index into array 'population' (holding all generations).</param>
function renderPopulation( populationIndex ) {
  ...
  for( var i=0; i < populationToRender.length; i++ ) {
    ...
    /* ... get its phenotype? Connect neurons?... */
    ...
    for( var j=0; j < fourierTransformTableSize; j++ ) {
      ...
      //var inputSignals = [Math.abs(extraInput)]; // d(istance), input
      // var inputSignals = [yInputSignal];
...
/* arg:memberIndex is an index into the array of waveforms currently on display
function returns a single waveform */
...
function evolveNextGeneration() {
  ...
  if( parentIndexes.length < 1 ) {
    ...
    // and then use that for a parent
...
*/
      ...
      $this.parent().find("span.computing-message").show( 100, function(){
        ...
        console.log("waveform "+ currentMemberIndex+ " was clicked");
...
/*
        // let's check the box in this container
        $this.find( "#member-" + currentMemberIndex ).attr( "checked", true );
*/
      });
  });
...
function Modulator( frequency, gain, detune ) {
  ...
  this.oscillator.setPeriodicWave( currentIndividualPeriodicWaves['modulator'] );

  // console.log(frequency + (frequency / 4) * modulatorQuarterOctaveOffset);

  this.oscillator.frequency.value = frequency + (frequency / 4) * modulatorQuarterOctaveOffset;
...
function Carrier( frequency ) {
  this.oscillator = audioContext.createOscillator();
  this.gain = audioContext.createGain();

  this.oscillator.setPeriodicWave( currentIndividualPeriodicWaves['carrier'] );
...
function createAndPlayModulatorsForFrequency( frequency, initialGain ) {
  ...
  var carrier = new Carrier( frequency );
  var modulator = undefined;

  if( shouldModulate ) {

    modulator = new Modulator( frequency, modulatorGain, modulatorDetune );
...
function playSelectedWaveformsForOneQuarterNoteC3( recordSample ) {
  ...
  if( useEnvelope ) {
    ...
    var noteOscillators = createAndPlayModulatorsForFrequency( frequency, 1 );
    window.setTimeout(function(){
...
var keyboard = new QwertyHancock( keyboardSettings );
...
/*
    ...
    oscillator.setPeriodicWave( currentIndividualPeriodicWaves['carrier'] );
...
*/
    ...
    notes.push( noteOscillators );
  }
};

function disconnectWaveOscillator( waveObject, recordSample ) {
  window.setTimeout(function(){
    ...
    if( waveObject.oscillator ) {
      waveObject.oscillator.stop(0);
      waveObject.oscillator.disconnect();
    }
  }, waveObject.releaseTime * 1000 );
}
...
keyboard.keyUp = function( note, frequency ) {
  ...
  for (var i = 0; i < notes.length; i++) {
      if( Math.round(notes[i]["carrier"].oscillator.frequency.value) === Math.round(frequency) ) {
        if( useEnvelope ) {

          if( notes[i]["modulator"] ) {
            notes[i]["modulator"].noteOff( audioContext.currentTime );
            disconnectWaveOscillator( notes[i]["modulator"] );
          }
          notes[i]["carrier"].noteOff( audioContext.currentTime );
          disconnectWaveOscillator( notes[i]["carrier"] );

        } else {

          if( notes[i]["modulator"] ) {
            notes[i]["modulator"].oscillator.stop(0);
            notes[i]["modulator"].oscillator.disconnect();
          }
          notes[i]["carrier"].oscillator.stop(0);
          notes[i]["carrier"].oscillator.disconnect();
        }
      } else {
...
function midiNotePlayedCallback( data ) {
...
var fourierTransformTableSize = 4096;  // maximum umber allowed according to the Web Audio API

var testOutputs = [];
for( var i=0; i < fourierTransformTableSize; i++ ) {

  var rangeFraction = i / (fourierTransformTableSize-1);
  var yInputSignal = lerp( -1, 1, rangeFraction );
